...
public class HorarioDao {
	...
	public void cadastrarHorario(Horario horario){
		...
		String sql = "insert into horario (idAgenda, idTurno, hora1, hora2, hora3, hora4) values (?,?,?,?,?,?)";
		...
		try {
			...
			if(horario.getHora1()==null){
				...
				stmt.setTime(3, horario.getHora1());
			}
			
			if(horario.getHora2()==null){
				stmt.setNull(4, 0);
			} else {
				stmt.setTime(4, horario.getHora2());
			}
			
			if(horario.getHora3()==null){
				stmt.setNull(5, 0);
			} else {
				stmt.setTime(5, horario.getHora3());
			}
			
			if(horario.getHora4()==null){
				stmt.setNull(6, 0);
			} else {
				stmt.setTime(6, horario.getHora4());
			}
	...
	public List<Horario> listarHorario(int idAgenda){
		...
		String sql = "select h.idTurno, t.nome from turno as t join horario as h on t.id=h.idTurno where h.idAgenda=?";
